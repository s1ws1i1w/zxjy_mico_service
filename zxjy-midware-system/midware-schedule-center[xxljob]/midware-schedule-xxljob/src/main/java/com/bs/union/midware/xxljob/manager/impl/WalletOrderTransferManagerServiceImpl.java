...
import com.bs.union.midware.xxljob.manager.WalletOrderTransferManagerService;
...
@Service
@Slf4j
public class WalletOrderTransferManagerServiceImpl implements WalletOrderTransferManagerService {
    ...
    public WalletOrderTransferManagerServiceImpl(WalletOrderService walletOrderService, WalletOrderInfoService walletOrderInfoService, WalletOrderDetailService walletOrderDetailService, CmsMerchantUserService merchantUserService, CmsMerchantService merchantService, CmsIndustryService industryService, StringRedisTemplate redisTemplate) {
    ...
    @Override
    public void walletOrderTransfer(Integer minId, Integer pageSize, Integer batch) {
        if (minId == -2) {
            String minIdStr = redisTemplate.opsForValue().get(RedisConstant.WALLET_ORDER_MIN_ID);
        ...
        Integer maxId = walletOrderService.selectMaxId();
        // 多线程去处理
        int batchSize = (maxId - minId) / batch;
        List<FutureTask<JSONObject>> futureTaskList = new ArrayList<>();
        for (int i = 0; i < batch; i++) {
            TransferOrderThread task = new TransferOrderThread(walletOrderService, walletOrderInfoService, walletOrderDetailService, merchantService, industryService, merchantUserService,i * batchSize + 1 , (i+1) * batchSize);
        ...
        if ((maxId - minId) % batch > 0) {
            TransferOrderThread task = new TransferOrderThread(walletOrderService, walletOrderInfoService, walletOrderDetailService, merchantService, industryService, merchantUserService,batch * batchSize + 1 , maxId);
            FutureTask<JSONObject> futureTask = new FutureTask<>(task);
            futureTaskList.add(futureTask);
            ScheduleTransferOrderThreadPoolManager.getExecutorManager().submit(futureTask);
        }
        ...
        redisTemplate.opsForValue().set(RedisConstant.WALLET_ORDER_MIN_ID, String.valueOf(maxId));
    }

    static class TransferOrderThread implements Callable<JSONObject> {
        private final WalletOrderService walletOrderService;
        private final WalletOrderInfoService walletOrderInfoService;
        private final WalletOrderDetailService walletOrderDetailService;
        private final CmsMerchantUserService merchantUserService;
        ...
        public TransferOrderThread(WalletOrderService walletOrderService, WalletOrderInfoService walletOrderInfoService, WalletOrderDetailService walletOrderDetailService, CmsMerchantService merchantService, CmsIndustryService industryService, CmsMerchantUserService merchantUserService, Integer minId, Integer maxId) {
            this.walletOrderService = walletOrderService;
            this.walletOrderInfoService = walletOrderInfoService;
            this.walletOrderDetailService = walletOrderDetailService;
            this.merchantUserService = merchantUserService;
        ...
        public JSONObject call() throws Exception {
            ...
            for (int i = 0; i < pages; i++) {
                ...
                if (walletOrderList.size() > 0) {
                    for (WalletOrderEntity walletOrderEntity : walletOrderList) {
                        JSONObject dtoObject = this.convertOrderInfoDTO(walletOrderEntity);
        ...
        /**
         * 实体类型转换
         * @param walletOrderEntity
         * @return
         */
        private JSONObject convertOrderInfoDTO(WalletOrderEntity walletOrderEntity) {
            if (walletOrderEntity.getStatus() == null) {
                XxlJobLogger.log(walletOrderEntity.getId() + "status为空");
                log.error("{}status为空", walletOrderEntity.getId());
                return null;
            }
            ...
            //5:退款中 6:退款成功 7:冻结 8:退款失败
            if (walletOrderEntity.getStatus() == 5 || walletOrderEntity.getStatus() == 6
                    || walletOrderEntity.getStatus() == 7 || walletOrderEntity.getStatus() == 8) {
                // 发生退款一定是有支付成功的记录
                info.setStatus(1);
            } else {
                info.setStatus(walletOrderEntity.getStatus());
            }
            JSONObject jsonObject = new JSONObject();
